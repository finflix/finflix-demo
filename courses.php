...
<body>
    ...
    <section class="iq-main-slider">
        <div id="tvshows-slider">
            ...
            ?>
                    <div>
                        <div href="videoPlayer.php?course=<?= $video_uuid ?>&module=<?= $module_id ?>">
                            <div class="shows-img">
                                ...
                                <div class="shows-content" style="margin-bottom:30px;">
                                    ...
                                    <div class="billboard-links button-layer forward-leaning d-flex justify-content-left">
                                        <a role="link" aria-label="Play – start with Audio Description turned on" class="visually-hidden ad-playlink" href="#">
                                            <span class="visually-hidden">&nbsp;</span>
                                        </a>
                                        <a role="link" aria-label="Play" class=" playLink isToolkit" href="videoPlayer.php?course=<?= $video_uuid ?>&module=<?= $module_id ?>">
    ...
    <div class="main-content">
        <?php
        $query = "SELECT * FROM `video_info` WHERE `module` = 'Tutorials'";
        $result = mysqli_query($con, $query);
        if (mysqli_num_rows($result) > 0) {
            $i = 1;
        ?>
        ...
        ?>
        <?php
        $query = "SELECT * FROM `video_info` WHERE `module` = 'Explainers'";
        $result = mysqli_query($con, $query);
        if (mysqli_num_rows($result) > 0) {
            $i = 1;
        ?>
            <section id="iq-favorites-3">
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-sm-12 overflow-hidden">
                            <div class="iq-main-header d-flex align-items-center justify-content-between">
                                <h4 class="main-title">Explainers</h4>
                            </div>
                            <div class="favorites-contens">
                                <ul class="favorites-slider list-inline  row p-0 mb-0 favorites-slider-new">
                                    <?php
        ...
        <?php } ?>
        <?php
        $query = "SELECT * FROM `video_info` WHERE `module` = 'Review'";
        $result = mysqli_query($con, $query);
        if (mysqli_num_rows($result) > 0) {
            $i = 1;
        ?>
        ...
        ?>
        <?php
        $query = "SELECT * FROM `video_info` WHERE `module` = 'News'";
        $result = mysqli_query($con, $query);
        if (mysqli_num_rows($result) > 0) {
            $i = 1;
        ?>
        ...
        <?php } ?>
        <?php
        $query = "SELECT * FROM `video_info` WHERE `module` = 'Others'";
        $result = mysqli_query($con, $query);
        if (mysqli_num_rows($result) > 0) {
            $i = 1;
        ?>
